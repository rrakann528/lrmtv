...
import { Search, Loader2, Film, Play, AlertCircle, ExternalLink, Monitor, Zap } from 'lucide-react';
import { apiFetch } from '@/hooks/use-auth';
import { useI18n } from '@/lib/i18n';
import CloudBrowser from './cloud-browser';
...
type SniffMode = 'auto' | 'manual';

export default function LinkSniffer({ onSelectVideo, roomSlug, socket }: LinkSnifferProps) {
  const [inputUrl, setInputUrl] = useState('');
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState<SniffedUrl[]>([]);
  const [error, setError] = useState('');
  const [duration, setDuration] = useState<number | null>(null);
  const [mode, setMode] = useState<SniffMode>('auto');
  const { lang } = useI18n();
    ...
    <div className="flex flex-col gap-3 h-full">
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={(e) => e.key === 'Enter' && !loading && mode === 'auto' && handleSniff()}
      ...
      {mode === 'auto' && (
        <>
          <button
            ...
            className="w-full px-4 py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all hover:brightness-110"
          >
            {loading ? (
              <>
